...
export const RecentlyScanned: React.FC<RecentlyScannedProps> = ({
  ...
  const handleScroll = (direction: 'left' | 'right') => {
    if (scrollRef.current) {
      // Scroll by approximately one full view container width
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Main Container: Sleek Rectangle Frame displaying 4 Square Product Cards at a time */}
      <div className="w-full bg-white/95 backdrop-blur-md rounded-3xl p-3 sm:p-4 border border-zinc-200/90 shadow-sm relative overflow-hidden">
        <div
          ref={scrollRef}
          className="flex items-stretch gap-3 sm:gap-4 overflow-x-auto pb-1 scrollbar-none snap-x snap-mandatory scroll-smooth"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {displayedItems.length === 0 ? (
            <div className="w-full py-12 text-center text-zinc-400 font-mono text-xs">
            ...
            displayedItems.map((item) => (
              <div
                ...
                // Calculated width: exactly 4 items visible with gaps (calc((100% - 3*gap) / 4)) on desktop/tablet, 2 on mobile
                className="snap-start shrink-0 w-[calc((100%-12px)/2)] sm:w-[calc((100%-36px)/3)] md:w-[calc((100%-48px)/4)] group bg-zinc-50/70 hover:bg-white rounded-2xl p-2.5 sm:p-3 border border-zinc-200/70 hover:border-zinc-300 hover:shadow-md active:scale-[0.98] transition-all cursor-pointer flex flex-col justify-between"
              >
                ...
                <div className="w-full aspect-square rounded-xl bg-white overflow-hidden relative border border-zinc-100 shadow-inner flex items-center justify-center">
                  ...
                  <span
                    className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-black shadow-xs ${item.gradeBg} ${item.gradeColor}`}
                  >
                    {item.grade}
                  </span>
                  ...
                  {item.is_database_record && (
                    <span className="absolute top-2 left-2 px-1.5 py-0.5 rounded-md bg-black/75 text-[#D5FF3F] text-[8px] font-mono font-bold tracking-wider backdrop-blur-xs">
                      DB
                    </span>
                  )}
                </div>
                ...
                <div className="pt-2 pb-1 space-y-0.5">
                  <h4 className="font-extrabold text-xs sm:text-[13px] text-zinc-900 leading-snug line-clamp-1 group-hover:text-indigo-600 transition-colors">
                    {item.name}
                  </h4>
                ...
                <div className="pt-1.5 border-t border-zinc-200/60 flex items-center justify-between">
                  ...
                  {onDeleteItem && item.is_database_record && (
                    <button
                      ...
                      className="p-1 rounded-md text-zinc-400 hover:text-rose-600 hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100"
                      title="Delete record"
                    >
          ...
          {/* Load More Card (if more specimens in database) */}
          {onLoadMore && hasMore && (
            <button
              onClick={onLoadMore}
              disabled={isLoadingMore}
              className="snap-start shrink-0 w-[calc((100%-12px)/2)] sm:w-[calc((100%-36px)/3)] md:w-[calc((100%-48px)/4)] bg-zinc-50/50 hover:bg-zinc-100/80 rounded-2xl p-3 border border-dashed border-zinc-300 hover:border-zinc-400 flex flex-col items-center justify-center text-center gap-2 text-zinc-600 transition-all active:scale-[0.98] cursor-pointer"
            >
              <div className="w-9 h-9 rounded-xl bg-white border border-zinc-200 flex items-center justify-center shadow-xs">
                <Plus className={`w-4 h-4 text-zinc-700 ${isLoadingMore ? 'animate-spin' : ''}`} />
              </div>
